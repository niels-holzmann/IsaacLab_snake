...
def design_scene():
    ...
    origins = [[0.25, 0.25, 0.0], [0.3, 0.3, 0.0], [0.25, -0.25, 0.0], [-0.25, -0.25, 0.0]]
    ...
    while simulation_app.is_running():
        ...
        if count % 250 == 0:
            ...
            root_state[:, :3] += math_utils.sample_cylinder(
                radius=0.1, h_range=(0.25, 0.5), size=sphere_object.num_instances, device=sphere_object.device
            )
            # write root state to simulation
            sphere_object.write_root_state_to_sim(root_state)
            # reset buffers
            sphere_object.reset()
            print("----------------------------------------")
        ...
        if count % 50 == 0:
            print(f"Root position (in world): {sphere_object.data.root_state_w[:, :3]}")
